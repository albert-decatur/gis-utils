...
# cubic spline using akima package
...
d.li <- interp(d$lon,d$lat,d$sshws, xo=seq(bbox(d)[1,1],bbox(d)[1,2], length=abs(bbox(d)[1,1] - bbox(d)[1,2])/pixelDivisor),yo=seq(bbox(d)[2,1],bbox(d)[2,2], length=abs(bbox(d)[2,1] - bbox(d)[2,2]/pixelDivisor)),linear=FALSE,extrap=TRUE)
